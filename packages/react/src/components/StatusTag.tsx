...
export function StatusTag({ status, loading }: { status: Status; loading?: boolean }) {
  if (loading) {
    return <span className="fp-skeleton" style={{ display: "inline-block", width: 70, height: 20, borderRadius: 4 }} />;
  }
  ...
  return (
    ...
    >
      <span style={{ width: 5, height: 5, borderRadius: "50%", background: cfg.color }} aria-hidden={true} />
      {cfg.label}
    </span>
  );
}
