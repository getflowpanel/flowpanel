...
describe("serializeWidget — table", () => {
  ...
  it("threads softDelete column through to adapter.list", async () => {
    ...
    await serializeWidget(widget, cfg, reqCtx, widgetCtx, new Request("http://localhost/"));
...
describe("serializeWidget — error & unknown", () => {
  ...
  it("catches query() errors and surfaces them as unsupported with the error message", async () => {
    const widget: WidgetConfig = {
      kind: "metric",
      label: "Boom",
      query: async () => {
    ...
    expect(out).toMatchObject({ kind: "unsupported", reason: "db down" });
  });
});
