...
function applyRowLevelFilter(config: FlowPanelContext["config"], session: any, whereParts: string[], params: unknown[]) {
...
export function createRunsProcedures(
  ...
  return t.router({
    list: authedProcedure
      ...
      .query(async ({ ctx, input }: { ctx: FlowPanelContext & { session: any }; input: z.infer<typeof listInputSchema> }) => {
        ...
        if (input.search && input.search.length >= 2) {
          const searchFields = (config as any).runLog?.search?.fields ?? ["partition_key", "error_message"];
          const searchCols = searchFields.map((f: string) => fieldNameToColumn(f));
          const searchOr = searchCols.map((col: string) => `${col} ILIKE $${params.length + 1}`).join(" OR ");
    ...
    get: authedProcedure
      ...
      .query(async ({ ctx, input }: { ctx: FlowPanelContext & { session: any }; input: { runId: string } }) => {
        const { db, config, session } = ctx;
    ...
    retry: authedProcedure
      ...
      .mutation(async ({ ctx, input }: { ctx: FlowPanelContext & { session: any }; input: z.infer<typeof retryInputSchema> }) => {
        const { db, config, session } = ctx;

        if (!(config as any).pipeline.onRetry) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Retry not configured. Add pipeline.onRetry to your flowpanel.config.ts" });
        }
        ...
        const run = rows[0];
        if (!run) throw new TRPCError({ code: "NOT_FOUND", message: "Run not found or not in failed status" });
    ...
    cancel: authedProcedure
      ...
      .mutation(async ({ ctx, input }: { ctx: FlowPanelContext & { session: any }; input: z.infer<typeof cancelInputSchema> }) => {
        const { db } = ctx;
    ...
    bulkRetry: authedProcedure
      ...
      .mutation(async ({ ctx, input }: { ctx: FlowPanelContext & { session: any }; input: z.infer<typeof bulkRetryInputSchema> }) => {
        const { db, config } = ctx;
        ...
        if (input.runIds) {
          if (input.runIds.length > 50 && !input.confirmText?.includes(String(input.runIds.length))) {
            throw new TRPCError({
              code: "BAD_REQUEST",
