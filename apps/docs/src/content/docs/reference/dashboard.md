---
title: 'Dashboard'
description: 'A **dashboard** is a grid of widgets that live-reload every 30 seconds. Widgets are pure async functions that run server-side; the client only receives data (never code), so secrets stay on the server'
---
