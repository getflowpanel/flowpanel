...
Cross-origin write protection is enabled by default. Configure `security.allowedOrigins` only when a trusted origin must submit writes; do not use it as authentication.
