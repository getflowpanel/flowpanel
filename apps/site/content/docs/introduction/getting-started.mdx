...
> `ai-scraper` example — Postgres in Docker, seeded, on `:3000`:
>
> ```bash
> git clone https://github.com/getflowpanel/flowpanel && cd flowpanel/examples/ai-scraper && pnpm install && pnpm docker:up && pnpm db:push && pnpm db:seed && pnpm dev
> ```
