...
`{ action: "create" | "update" | "delete", id? }`. On the wire (both the
Redis pub/sub message and the SSE frame the browser receives) that payload
is wrapped in a `{channel, payload}` envelope, so a subscriber can route on
`channel` without inspecting payload shape (ADR 0014).
...
Redis wire; it never changes the name application code sees:

```
flowpanel:resource.orders → {"channel":"resource.orders","payload":{"action":"update","id":"ord_…"}}
flowpanel:resource.users  → {"channel":"resource.users","payload":{"action":"create","id":"usr_…"}}
flowpanel:scraper:tick    → {"channel":"scraper:tick","payload":<arbitrary payload via publish("scraper:tick", …)>}
```

(With `keyPrefix: "fp"` those become `fp:resource.orders`, `fp:resource.users`,
`fp:scraper:tick` — same logical channels, different wire prefix.)
