...
`table({ realtime: "resource.orders" })`, and `publishResource(name, …)`
call now fans out via Redis instead of an in-process EventEmitter. (A
resource's `realtime` is `boolean | string`; a widget's is
`string | string[]` — a widget names the channels it listens to, so
`true` is not a value it accepts.)
