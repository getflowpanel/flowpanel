...
**Cause:** the admin route layout does not import the generated `admin.css`, or a local override prevents the precompiled FlowPanel stylesheet from loading.

**Fix:** restore the generated `@import "@flowpanel/kit/styles/admin.css";` line. The default stylesheet does not require Tailwind scanning or a PostCSS plugin. If your project intentionally uses the legacy `@flowpanel/react/styles/admin.css` source export, restore its own Tailwind source/content configuration instead.
...
**Cause:** the worker publishes through an in-memory publisher, or uses a different Redis prefix.

**Fix:** publish through `createPublisher({ redis: process.env.REDIS_URL! })` from `@flowpanel/kit/publish`, with the admin's `keyPrefix`.
