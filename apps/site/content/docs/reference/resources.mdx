...
`detail.header(row)` supplies the content of the detail page's `h1`. Return text
or inline React elements, rather than another heading or a complete page header.
The callback runs on the server and receives the same authorized row projection
as detail tabs: declared readable fields and permitted `expose` fields. It never
receives undeclared adapter fields or fields denied by `fieldAccess`.

Omit the callback, or return `null`/`undefined`, to keep the default resource label
and readable row identifier. The generated edit action remains beside the heading;
it is omitted when `update.disabled` is true or the resource's update access policy
denies the request, including asynchronous policies. Set `labels.actions.edit` to
translate its text. The edit route enforces its own authorization as well.

Custom `PageHeader` slots should use the exported `PageHeaderProps` type: `title`
accepts React content as well as strings.
...
Only the selected tab runs on the server, and the tab in view is `?tab=<key>`.
A tab with a `resource` renders that resource's list columns, sorted by its
`defaultSort` and otherwise by its primary key, and paginates under its own URL
key `relatedPage.<key>` — so two tabs page independently and Back, Forward and
reload restore the page the reader was on. The tab's `filter` is a constraint,
not a refinement: an undefined parent value or a field the reader may not read
yields no rows and no query.
