...
every adapter call and every action `run` in it. This is the one piece of
ambient state the framework keeps, and invariant I-10 confines it to
AsyncLocalStorage precisely so it stays request-scoped rather than global.
