...
| `BooleanFilter`, `DateRangeFilter`, `MultiSelectFilter`, `NumericRangeFilter`, `SelectFilter`, `TagFilter`, `TextFilter` | One filter control per `ColumnMeta.type`, each driven by `FilterBar`. |
| `DateRangePicker` | Standalone from/to date range control. |
...
route's search params; `useAdminTable` and `useAdminDrawer` are both built on
it.
...
flag. It is the current inline-edit primitive; the 1.x spec plans a dedicated
debounce layer on top of it for rapid successive edits (typing into an
inline-edit cell), not yet built.
...
provider), `debounceMs` and `endpoint` apply directly and it opens its own
`EventSource` per channel.
