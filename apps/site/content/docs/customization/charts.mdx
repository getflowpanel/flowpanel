...
Use series labels instead of exposing database keys. Apply number, currency, percentage, and date formatting consistently with nearby metrics. The default palette comes from `--fp-chart-1` through `--fp-chart-7`; override those tokens for brand alignment and verify contrast in light and dark modes.
...
export function Preview({ data }: { data: Array<{ day: string; count: number }> }) {
  return <LineChart data={data} xKey="day" series={[{ key: "count", label: "Count" }]} />;
}
```
