...
The interface is declared by `@flowpanel/core` and augmented by
`@flowpanel/react` with the shipped 10 — both reachable as `@flowpanel/kit`
and `@flowpanel/kit/react`. Your override for `Button`
